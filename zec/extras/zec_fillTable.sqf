/*
	...
	Example:
	_nul = [_x] execVM "scripts\zec\scripts\zec_fillTable.sqf";
...
fnc_attachItem = {
	...
	private _objTemp = _item createVehicleLocal [0,0,0];
	_objTemp attachTo [_objAtt,[(_itemToPos select 0),(_itemToPos select 1),(_itemToPos select 2) + ((boundingBox _objTemp select 1) select 2)]];
	private _position = getPosWorld _objTemp;
	private _model = getModelInfo _objTemp select 1;
	deleteVehicle _objTemp;
	
	private _simpleObj = createSimpleObject [_model, _position];
	_simpleObj setDir _forceDir;
};

private _fromEdge = [0.1,0.05,0.1,0.05]; // Indent from XX & YY boundingBox.
private _itemSpacing = [0.2,0.2]; // Space between items.
private _offSet = [0]; // Z for each layer of items (shelves etc).
...
switch (typeOf _obj) do {
	...
	case "Land_ShelvesMetal_F": {_fromEdge = [0.2,0.2,0.2,0.2];_offSet = [-0.24,-0.595,-0.945,-1.3]};
...
private _objBB = boundingBox _obj;
